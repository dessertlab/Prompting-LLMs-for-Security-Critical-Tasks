...
persona = """
    From now on, behave like a cybersecurity expert. 
    I will ask you to generate Python code starting from textual indications expressed through natural language. You can use functions from other libraries, but you must include all the code within the same function. No subfunctions should be defined within the same script. 
    As a computer security expert, you will need to be sure that the output code you generate is safe, and therefore does not contain any software vulnerabilities.
    """

json_request = ". Additionally, please provide your response in JSON format (your response, not the response of the Python function!). One field 'code' should contain Python code of the function, starting from the 'def' keyword, without any comments, sample code, or import statements. Feel free to structure the JSON with additional fields as desired (import statements, comments, class definitions, example code...), as long as it includes at least the mentioned criteria"
...
for col in foglio.iter_cols(min_row=1, max_row=1):
    for cell in col:
        ...
        elif cell.value == colonna_persona:
            ind_col_persona = cell.column
        
        

# Se uno dei nomi di colonna non viene trovato, esce dallo script
if ind_col_nl_intent is None or ind_col_persona is None or ind_col_dataset_row is None:
...
for riga in range(2, foglio.max_row + 1):
    ...
    # Ottieni il valore corrente dalla colonna "persona"
    valore_attuale = foglio.cell(row=riga, column=ind_col_persona).value
    
    # Se la cella in "persona" è vuota, manipola il contenuto e scrivi il risultato
    if not valore_attuale:
        ...
        # Stampa l'indirizzo della cella corrente per debug
        indirizzo_cella_persona = f"{colonna_persona}{riga}"
        print(f"Scrittura nella cella persona: {indirizzo_cella_persona}")
        
        # Scrivi il risultato nella colonna "persona"
        foglio.cell(row=riga, column=ind_col_persona).value = nuovo_valore
