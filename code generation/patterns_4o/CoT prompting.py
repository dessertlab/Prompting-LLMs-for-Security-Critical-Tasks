...
colonna_cot_prompting = "CoT Prompting"
...
for col in foglio.iter_cols(min_row=1, max_row=1):
    for cell in col:
        ...
        elif cell.value == colonna_cot_prompting:
            ind_col_cot_prompting = cell.column

# Se uno dei nomi di colonna non viene trovato, esce dallo script
if ind_col_nl_intent is None or ind_col_cot_prompting is None:
...
for riga in range(2, foglio.max_row + 1):
    ...
    # Ottieni il valore corrente dalla colonna "cot_prompting"
    valore_attuale = foglio.cell(row=riga, column=ind_col_cot_prompting).value
    
    # Se la cella in "cot_prompting" è vuota, manipola il contenuto e scrivi il risultato
    if not valore_attuale:
        ...
        # Stampa l'indirizzo della cella corrente per debug
        indirizzo_cella_cot_prompting = f"{colonna_cot_prompting}{riga}"
        print(f"Scrittura nella cella cot_prompting: {indirizzo_cella_cot_prompting}")
        
        # Scrivi il risultato nella colonna "cot_prompting"
        foglio.cell(row=riga, column=ind_col_cot_prompting).value = nuovo_valore
