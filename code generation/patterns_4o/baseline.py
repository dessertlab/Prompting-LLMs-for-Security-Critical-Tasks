...
file_excel = os.path.join(current_dir, "..", "VP Code - Dataset.xlsx")
...
for col in foglio.iter_cols(min_row=1, max_row=1):
    for cell in col:
        ...
        elif cell.value == colonna_baseline:
            ind_col_baseline = cell.column

# Se uno dei nomi di colonna non viene trovato, esce dallo script
if ind_col_nl_intent is None or ind_col_baseline is None:
...
for riga in range(2, foglio.max_row + 1):
    ...
    # Ottieni il valore corrente dalla colonna "baseline"
    valore_attuale = foglio.cell(row=riga, column=ind_col_baseline).value
    
    # Se la cella in "baseline" è vuota, manipola il contenuto e scrivi il risultato
    if not valore_attuale:
        ...
        # Stampa l'indirizzo della cella corrente per debug
        indirizzo_cella_baseline = f"{colonna_baseline}{riga}"
        print(f"Scrittura nella cella baseline: {indirizzo_cella_baseline}")
        
        # Scrivi il risultato nella colonna "baseline"
        foglio.cell(row=riga, column=ind_col_baseline).value = nuovo_valore
